<?php /* === EVENT TYPES HORIZONTAL SCROLL === */ ?>
...
<section id="<?= esc($d['id'] ?: 'eventos') ?>" class="tb-section <?= esc($d['scheme']) ?> <?= anim_attrs($d) ?>">
  <div class="container px-0 px-md-3">
    ...
    <div class="tb-event-types__scroll">
      <?php foreach ($data['items'] ?? [] as $item): ?>
        <div class="tb-event-types__scroll-card flex-shrink-0">
          <div class="tb-card">
            <img class="tb-card__img" src="<?= esc($item['img'] ?? placeholder('600x400', 'f3dfc8', '7f5539', ($item['title'] ?? 'Evento'))) ?>" alt="<?= esc($item['img_alt'] ?? $item['title'] ?? '') ?>">
