          ...
          <?php $i = 0; foreach ($d['items'] ?? [] as $item): $i++; ?>
            <div class="accordion-item mb-3 rounded-3 overflow-hidden border-0 shadow-sm">
              <h3 class="accordion-header">
                <button class="accordion-button tb-faq-btn tb-faq--1__button <?= $i > 1 ? 'collapsed' : '' ?> fw-semibold rounded-3 py-3 px-4"
                        type="button" data-bs-toggle="collapse" data-bs-target="#faq<?= $i ?>"
                        aria-expanded="<?= $i === 1 ? 'true' : 'false' ?>" aria-controls="faq<?= $i ?>">
                  <?php if (!empty($item['icon'])): ?>
                    <span class="tb-faq-icon tb-faq--1__icon"><i class="bi <?= esc($item['icon']) ?>"></i></span>
                  <?php endif; ?>
                  <span class="tb-faq--1__question flex-grow-1"><?= esc($item['q'] ?? '') ?></span>
                </button>
              </h3>
