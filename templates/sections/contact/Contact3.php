          ...
          <div class="d-grid gap-3">
            <?php foreach ($data['fields'] ?? [] as $field): ?>
              <?php if (($field['type'] ?? 'text') === 'select'): ?>
                <select class="form-select" id="<?= esc($field['id']) ?>" style="padding:1rem 1.25rem; border-radius:12px">
              ...
              <?php elseif (($field['type'] ?? 'text') === 'textarea'): ?>
                <textarea class="form-control" id="<?= esc($field['id']) ?>" rows="<?= esc((string)($field['rows'] ?? 4)) ?>"
                          ...
                          style="padding:1rem 1.25rem; border-radius:12px"></textarea>
              <?php else: ?>
                <input type="<?= esc($field['type'] ?? 'text') ?>" class="form-control" id="<?= esc($field['id']) ?>"
                       placeholder="<?= esc($field['placeholder'] ?? '') ?>"
                       style="padding:1rem 1.25rem; border-radius:12px"
                       <?= !empty($field['required']) ? 'required' : '' ?>>
              <?php endif; ?>
            <?php endforeach; ?>
            <button type="submit" class="tb-btn tb-btn--primary tb-btn--lg w-100" style="border-radius:50px; padding:1rem">
